...
from evaluation.eval_datasets import IRDataset, SimpleDataset
...
class SciRepEval:
    def __init__(
        self,
        tasks_config: str = "scirepeval_tasks.jsonl",
    ...
    def evaluate(self, model: Union[Model, List[Model]], output: str):
        ...
        for task_name, task in self.tasks.items():
            ...
            if task_data.get("meta"):
                ...
                kwargs["meta_dataset"] = (
                    metadata if type(metadata) != dict else (metadata["name"], metadata["config"])
                )
            ...
            if task_data.get("test"):
                ...
                kwargs["test_dataset"] = (
                    testdata if type(testdata) != dict else (testdata["name"], testdata["config"])
                )
            ...
            if task["type"] in {"classification", "regression"}:
                subtype = (
                    SupervisedTask.CLASSIFICATION
                    if task["type"] == "classification"
                    else SupervisedTask.REGRESSION
                )
                if task.get("multi_label"):
                    subtype = SupervisedTask.MULTILABEL_CLASSIFICATION
                evaluator = SupervisedEvaluator(task_name, subtype, model=model, **kwargs)
                if task.get("few_shot"):
                    for run in task["few_shot"]:
                        few_shot_evaluators.append(
                            FewShotEvaluator(
                                f"{task_name} {run['sample_size']} shot",
                                subtype,
                                model=model,
                                sample_size=run["sample_size"],
                                num_iterations=run["iterations"],
                                **kwargs,
                            )
                        )
            else:
                if task_name == "Paper-Reviewer Matching":
                    if not task_data.get("reviewers") and not task_data.get("hf_reviewers"):
                        raise ValueError(
                            f"Task {task_name} has no reviewer metadata locally or hf_metadata"
                        )
                    if task_data.get("reviewers"):
                        reviewers = task_data["reviewers"]
                        kwargs["reviewer_metadata"] = (
                            reviewers
                            if type(reviewers) != dict
                            else (reviewers["name"], reviewers["config"])
                        )
                    ...
                    evaluator = IREvaluator(
                        task_name, model=model, dataset_class=data_class, **kwargs
                    )
            ...
            for few_shot in few_shot_evaluators:
                final_results[task_name]["few_shot"].append(
                    {
                        "sample_size": few_shot.sample_size,
                        "results": few_shot.evaluate(embeddings),
                    }
                )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tasks-config", help="path to the task config file", default="scirepeval_tasks.jsonl"
    )
    parser.add_argument(
        "--mtype",
        help="Model variant to be used (default, pals, adapters, fusion)",
        default="default",
    )
    parser.add_argument(
        "--gpt3-model", help="Name of embedding model in case of using openai api", default=None
    )
    parser.add_argument("--model", "-m", help="HuggingFace model to be used")
    parser.add_argument("--batch-size", type=int, default=32, help="batch size")
    parser.add_argument(
        "--ctrl-tokens", action="store_true", default=False, help="use control codes for tasks"
    )
    ...
    parser.add_argument(
        "--adapters-chkpt", help="hf adapter names keyed on tasks", default=None, type=json.loads
    )
    parser.add_argument(
        "--output", help="path to the output file", default="scirepeval_results.json"
    )
    parser.add_argument(
        "--fp16", action="store_true", default=False, help="use floating point 16 precision"
    )
    parser.add_argument(
        "--instructor", action="store_true", default=False, help="use an instructor model for eval"
    )
    parser.add_argument(
        "--load_in_4bits",
        action="store_true",
        default=False,
        help="use 4 bits inference for mistral model",
    )
